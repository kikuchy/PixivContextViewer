...
pcv.contextViewer.MemberBookmarkContext = (function($, _super){
  ...
  var urlTmpl = function(pageNum, memberId){
    return "http://www.pixiv.net/bookmark.php?id=" + memberId + "&rest=show&p=" + pageNum;
  };
  ...
  MemberBookmarkContext.prototype.getNextAsync = function(){
    ...
    $.when(this.getHasNextAsync()).done(function(has){
      ...
      var ret = new MemberBookmarkContext(urlTmpl(self._nextNumOfPage, self.memberId));
      dfd.resolve(ret);
    });
    return dfd.promise();
  };
  ...
  MemberBookmarkContext.prototype.getPrevAsync = function(){
    ...
    $.when(this.getHasPrevAsync()).done(function(has){
      if(!has) dfd.resolve(null);
      var ret = new MemberBookmarkContext(urlTmpl(self._prevNumOfPage, self.memberId));
      dfd.resolve(ret);
    });
    return dfd.promise();
  };
