...
pcv.contextViewer.View = (function($){
  ...
  View.prototype.initializeView = function(context){
    ...
    $(document).ready(function(){
      $("body").append(self.$pcvWindow);
      $("#pcv-contents").css("width", self.contentWidth + "px");
      self.centeringListToHere(hereIdx);
      self.$pcvWindow.trigger("scroll");
      self.$pcvWindow.removeClass("invisible");
    });
  };
  ...
  View.prototype.addContextToHead = function(context){
    ...
    var prevPos = $("#pcv-contents", this.$pcvWindow).offset();
    this.contentWidth += context.pages.length * PAGE_WIDTH;
    $("#pcv-contents").css("width", this.contentWidth + "px");
    this.$pcvWindow.scrollLeft(-prevPos.left + context.pages.length * PAGE_WIDTH);
  };
