...
pcv.contextViewer.BackController = (function(){
  ...
  BackController.prototype.getNextContextAsync = function(nowCtx){
    var dfd = $.Deferred();
    var ctx = this._checkContext((nowCtx.url || this._restoreContext(this.tabId)) || "");
    if(!ctx) return dfd.reject();
    var self = this;
    $.when(ctx.getNext()).done(function(nextCtx){
      $.when(nextCtx.getPages(), nextCtx.getHasPrev(), nextCtx.getHasNext()).done(function(pages, hasPrev, hasNext){
        var ret = {
          pages: pages,
          hasPrev: hasPrev,
          hasNext: hasNext,
          numOfPage: nextCtx.pageOf,
          url: nextCtx.url
        };
        dfd.resolve(ret);
      });
