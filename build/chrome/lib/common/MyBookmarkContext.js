...
pcv.contextViewer.MyBookmarkContext = (function($, _super){
  var EXP_PAGEOF = /p=(\d+)/;
  var EXP_ID = /id=(\d+)/;
  var EXP_REST = /rest=(show|hide)/;
  ...
  var urlTmpl = function(pageNum, rest, order){
    return "http://www.pixiv.net/bookmark.php?rest=" + rest + "&order=" + order + "&p=" + pageNum;
  };

  var MyBookmarkContext = function(url){
    _super.apply(this, [url]);
    this.url = url;
    var params = url.substring(url.indexOf("?") + 1);
    var m = params.match(EXP_PAGEOF);
    this.pageOf = (m)? parseInt(m[1]) : 1;
    m = params.match(EXP_REST);
    this.rest = (m)? m[1] : "show";
  ...
  MyBookmarkContext.prototype = $.extend({}, _super.prototype);

  MyBookmarkContext.prototype._parse = function(html){
  ...
  MyBookmarkContext.prototype._makeParamsAsync = function(url){
    var self = this;
  ...
  MyBookmarkContext.prototype._getParamAsync = function(param){
    var dfd = $.Deferred();
  ...
  MyBookmarkContext.prototype.getPagesAsync = function(){
    return this._getParamAsync("pages");
  };

  MyBookmarkContext.prototype.getTotalPagesAsync = function(){
    return this._getParamAsync("totalPages");
  };

  MyBookmarkContext.prototype.getHasNextAsync = function(){
    return this._getParamAsync("hasNext");
  };

  MyBookmarkContext.prototype.getHasPrevAsync = function(){
    return this._getParamAsync("hasPrev");
  };

  MyBookmarkContext.prototype.getNextAsync = function(){
    var dfd = $.Deferred();
    var self = this;
    $.when(this.getHasNextAsync()).done(function(has){
      if(!has) dfd.resolve(null);
      var ret = new MyBookmarkContext(urlTmpl(self._nextNumOfPage, self.rest, self.order));
      dfd.resolve(ret);
    });
    return dfd.promise();
  };

  MyBookmarkContext.prototype.getPrevAsync = function(){
    var dfd = $.Deferred();
    var self = this;
    $.when(this.getHasPrevAsync()).done(function(has){
      if(!has) dfd.resolve(null);
      var ret = new MyBookmarkContext(urlTmpl(self._prevNumOfPage, self.rest, self.order));
      dfd.resolve(ret);
    });
    return dfd.promise();
  };

  return MyBookmarkContext;
